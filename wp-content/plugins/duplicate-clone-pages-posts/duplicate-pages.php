...
/**
 * Plugin Name: Duplicate / Clone Pages and Posts
 * Plugin URI: http://itspiders.net/
 * Description: Duplicate / Clone ( Single or in a Bulk ) Posts and Pages with one click.
 * Version: 1.0
 * Author: It Spiders
...
/*  Copyright 2015  It Spiders  (email : info@itspiders.net)
...
if ( !class_exists( 'DuplicatePages' ) and is_admin()) :
...
class DuplicatePages {
	...
	function admin_footer() {
		...
		<script type="text/javascript">
			jQuery(function () {
				jQuery('<option>').val('clone').text('<?php _e('Duplicate')?>').appendTo("select[name='action']");
				jQuery('<option>').val('clone').text('<?php _e('Duplicate')?>').appendTo("select[name='action2']");
			});
		</script>
		<?php
	}
	...
	function action() {
		...
		switch ( $action ) {
			case 'clone':
				...
				$sendback = add_query_arg( array( 'duplicated' => $cloned, 'ids' => join(',', $post_ids) ), $sendback );
				break;
	...
	function admin_notices() {
		...
		if ($pagenow == 'edit.php' && ! isset($_GET['trashed'] )) {
			...
			if ($cloned) {
				$message = sprintf( _n( 'Page Duplicated.', '%s pages Duplicated.', $cloned ), number_format_i18n( $cloned ) );
	...
	function post_row_actions( $actions, $post ) {
		...
		$actions['clone'] =  '<a href=\''.$url.'\'>'.__('Duplicate').'</a>';
		return $actions;
	}
